...
InvSIM800::InvSIM800()
{
	...
	DDRD |= 1<<SIM800_RS; //SIM800_RS // reset pin -> output
	urc_status = 0xff;
	hostname = 0;
	param = 0;
...
void InvSIM800::switchEventSync(EventSync sync)
{
	eventsync = sync;
}
...
{
	volatile static uint8_t retries = 0;
	retries++;
	if (retries > 5)
	{
		if (shutdown())
		{
			switchEventSync(SYNC_WAKEUP);
		}
		retries = 0;
	}
...
//
	if (!_sim_is_waked && _reset_delay == 20)
	{
...
void InvSIM800::setup()
{
	switch(eventsync)
	{
		case SYNC_WAKEUP:
			...
			break;
		case SYNC_REG_NTWK:
			if(registerNetwork(3000)) switchEventSync(SYNC_SET_APN);
			break;
		case SYNC_SET_APN:
			setSwitchAPN();
			break;
		case SYNC_ENABLE_GPRS:
			if(enableGPRS()) switchEventSync(SYNC_HTTP_REQUEST);
			break;
...
{
// 	serialWriteString(0, "Debugging\n");
// 	serialWriteString(0, getUrl());

	if (_request_delay > 30)
	{
		...
		{
			char buffer[20] = {0};
			uint16_t idx = HTTP_read(buffer, 0, 4);
			if (idx)
			{
